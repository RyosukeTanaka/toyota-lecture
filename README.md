...
3.  **バッチ分析:** 複数日付・車両クラスの組み合わせに対して一括で予測分析を行い、集計結果を表示します。予測値は四捨五入されて計算・表示されます。
...
1.  **データ読み込みと初期処理:**
    ...
    *   日付列の型変換、リードタイム（予約日から利用日までの日数）の計算を行います。
    *   価格差・価格比などの特徴量エンジニアリングは、`PRICE_COLUMNS` 定数に2つ以上の価格列が定義されている場合に実行されます（現在は `価格_トヨタ` のみのため、実質これらの特徴量はNaNとなります）。
...
2.  **実績データの可視化:**
    ...
    *   同期間の価格推移（現在は `価格_トヨタ` のみ）もグラフで表示します。
3. **予約曲線比較の表示オプション:**
    *   **タブインターフェース**で「価格変更後のみ」と「全期間比較」を切り替え可能です。
    *   「全期間比較」タブでは価格変更点を**緑色の縦線**でマークし、変更前後の傾向を一度に確認できます。
4.  **シナリオベースの予測:**
    *   実績データから、選択された**利用日**に対して `価格_トヨタ` が最後に変更されたリードタイムを検出します。
    *   「もし `価格_トヨタ` が最後に変更された時点から利用日まで、その最後の価格で固定されていたら」というシナリオを作成します。
    *   選択されたモデルを用いて、このシナリオに基づいた利用台数の予測値を計算します。（`価格_トヨタ` に変動がなかった場合は実行されません。）
...
5.  **実績 vs 予測の比較:**
    *   `価格_トヨタ` に価格変動があった場合、実際の利用台数と上記シナリオに基づく予測値を比較するグラフとデータテーブルを表示します。
    *   比較は、`価格_トヨタ` が最後に変更されたリードタイム以降の期間に限定して表示されます。
6.  **売上金額ベースの比較分析:**
    *   実績と価格固定シナリオそれぞれの「利用台数×`価格_トヨタ`」に基づく売上金額を計算します。
...
3.  **集計結果の表示:**
    ...
    *   日次売上差額、累積売上差額、`価格_トヨタ` の価格変更点情報などを一覧できます。
    *   「追加予測予約数（価格が変更されなかった場合）」という列名で、価格固定シナリオでの追加予約数を表示します。
    *   予測関連の数値は四捨五入されて表示・計算されます。
4.  **売上金額影響分析:**
    *   `価格_トヨタ` の変更が売上に与えた影響を「各リードタイムでの日次新規予約数×`価格_トヨタ`」の計算方法で定量的に評価します。
    *   実績売上と予測売上（`価格_トヨタ` 固定シナリオ）の差額に基づく影響分析を行います。
...
3.  **データ更新とラグ再計算:**
    ...
    *   ユーザーは複数のラグ日数（7日、15日、30日）から再計算する特徴量を選択できます。
    *   データ更新後、影響を受けるラグ特徴量（例: `利用台数累積_lag7`、`利用台数累積_lag15`、`利用台数累積_lag30`）を自動で再計算します。
...
*   **画面表示:**
    ...
    *   【予測ページ】 実績の予約曲線・価格推移グラフ（タブで全期間/価格変更後を切替、価格推移は `価格_トヨタ` のみ）
    *   【予測ページ】 予測時の特徴量欠損値補完ログ、補完前後のNaN行データ
    *   【予測ページ】 実績 vs 予測比較グラフとテーブル (`価格_トヨタ` に変動があった場合)
    *   【予測ページ】 売上金額ベースでの差額分析グラフとメトリクス (基準は `価格_トヨタ`)
    *   【バッチページ】 複数日付・車両クラスの集計結果一覧 (予測値は四捨五入、列名変更反映)
    *   【バッチページ】 日付別・車両クラス別の売上差額分析 (基準は `価格_トヨタ`)
